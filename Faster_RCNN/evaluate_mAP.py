...
def evaluate_custom_mAP(predictions, ground_truths, iou_threshold = 0.5, iou_function : str = 'GIoU'):
    """
    Calculating mAP for predicted boxes and ground truth labels using a custom iou_function.
    Implemented IoU Functions:
    1. General Intersection Over Union (GIoU)
    2. Distance-IoU (DIoU)
    3. Complete-IoU (CIoU)
    4. Soft-IoU (SIoU)

    Parameters:
    - predictions: predictions in json format
    - ground_truths: COCO ground truths file
    - iou_threshold: the IoU threshold for making decisions for mAP (Default: 0.5)
    - iou_function: the custom IoU function for making decision for mAP (Default: GIoU)
...
def compute_ap(predictions, ground_truths, iou_threshold = 0.5, iou_function : str = 'GIoU'):
    """
    Calculating mAP for predicted boxes and ground truth labels using a custom iou_function.
    Implemented IoU Functions:
    1. General Intersection Over Union (GIoU)
...
if __name__ == '__main__':
    # Predictions
    predictions = [
        {'image_id': 1, 'category_id': 1, 'bbox': [50, 50, 100, 100], 'score': 0.95},
        {'image_id': 1, 'category_id': 2, 'bbox': [150, 150, 50, 50], 'score': 0.88}
    ]   

    # Ground truths
    ground_truths = [
        {'image_id': 1, 'category_id': 1, 'bbox': [60, 60, 80, 80]},
        {'image_id': 1, 'category_id': 2, 'bbox': [160, 160, 50, 50]}
    ]

    # Calculating mAP
    mAP = evaluate_custom_mAP(predictions, ground_truths, iou_function = 'CIoU')
    print("mAP:", mAP)